use actix_web::{
    ...
    web::{Data, Json, scope, Path, ServiceConfig, Query},
...
async fn create_client(
    ...
    println!("Recebido pedido para criar cliente: {:?}", body);
    ...
    {
        Ok(client) => {
            println!("Cliente criado com sucesso: {:?}", client);
        ...
        Err(error) => {
            println!("Erro ao criar cliente: {:?}", error);
