...
async fn create_product(
    ...
    match sqlx::query_as::<_, ProductModel>(query)
        .bind(&body.name)
        .bind(&body.description) // Use reference for optional
        .bind(body.price.clone()) // Clone BigDecimal for binding
        .bind(body.stock_quantity) // Bind the stock_quantity directly
        .fetch_one(&**db_pool)
        .await
    {
